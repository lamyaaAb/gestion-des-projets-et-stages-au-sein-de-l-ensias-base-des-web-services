...
<form action="affecter_note.php" method="post">
  ...
  <select style="margin-left:115px;" class="btn btn-primary dropdown-toggle " name="nomP" class="dropdown-menu dropdown-menu-lg-end ">
  <option disable >Le sujet du projet</option>
    <?php  
           $stmt5=$db->prepare('select * from projet where idProjet in (select idProjet from soutenance where idJury =(select idJury from membreJury where idMembreJury=:idmj) )');
          $stmt5->execute(array('idmj'=>$_SESSION['id_mj']));
