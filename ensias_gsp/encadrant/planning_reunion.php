...
if(isset($_POST['programmer']) &&isset($_POST['heure']) && isset($_POST['duree']) && isset($_POST['nomP']) && isset($_POST['ordre']) && isset($_POST['date']))
{
  $duree=$_POST['duree'];
  $date=$_POST['date'];
  ...
  while($row1=$stmt3->fetch(PDO::FETCH_ASSOC))
  {
    extract($row1);
    $id=$idProjet;
    $stmt=$db->prepare('INSERT into reunion(date_reunion,duree_reunion,ordre_reunion,idProjet,heure) values(:dater,:dureer,:ordrer,:idProjet,:heure)');
    $stmt->execute(array(
              'dater' =>$date,
              'dureer'=> $duree,
              'ordrer'=>$ordre,
...
//recupération des evenements des projets 
     ...
     $stmt2=$db->prepare('select * from reunion where MONTH(date_reunion)=MONTH(CURDATE()) and idProjet in (select idProjet from projetencadrant where idEncadrant=:idenc)');
     $stmt2->execute(array( 
                    'idenc'=>$_SESSION['id_encadrant']));
      while($row3=$stmt2->fetch(PDO::FETCH_ASSOC))
        {
          ...
          $calendar->add_event($ordr.' à '.$h, $dte, 1, 'green');      
        }
...
<body>
  ...
  <br>
<form action="planning_reunion.php" method="post">
  <div class="row mb-7">
  ...
  <div class="row mb-7">
    <label for="inputText" class="col-sm-2 col-form-label">L'heure de la réunion</label>
        <div class="col-sm-10">
  ...
  <select style="margin-left:200px;" class="btn btn-primary dropdown-toggle " name="nomP" class="dropdown-menu dropdown-menu-lg-end ">
 ...
 <br><br>
  <div class="text-center"><button class="btn btn-primary" name="programmer" type="submit">Programmer la réunion</button></div>
  <br><br><br>
</form>
